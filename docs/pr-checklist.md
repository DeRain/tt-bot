...
- [ ] Mutation tests pass (`make mutation-test-pr`)
...
4. **Gates**: `make gate-all` passes — includes build, lint, test, check-coverage, arch-check, and mutation testing on changed lines.
5. **Mutation**: `make mutation-test-pr` produces no surviving mutants on changed lines.
6. **Docs**: traceability.md and verification.md are updated.
